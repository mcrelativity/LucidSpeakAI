...
export default function PricingPage() {
    ...
    return (
        <div className="flex-grow flex flex-col items-center justify-center w-full text-center p-6">
            <h1 className="text-4xl font-bold text-sky-400 mb-6">{t('title')}</h1>
            <div className="w-full max-w-md bg-slate-800 p-10 rounded-lg shadow-lg">
                <h2 className="text-2xl font-bold">{t('planName')}</h2>
                <p className="text-5xl font-bold my-4">{t('price')} <span className="text-lg text-slate-400">{t('currency')}</span></p>
                <p className="text-slate-400 mb-2">{t('billingPeriod')}</p>
                <p className="text-sm text-slate-500 mb-8">{t('cancelAnytime')}</p>
                ...
                <div className="min-h-[150px]">
                        ...
                        {!isPending && (
                            <>
                                <PayPalButtons
                                    ...
                                    disabled={!user}
                                />
                                {!user && <p className="text-yellow-400 text-sm mt-4">{t('loginRequired')}</p>}
                            </>
                        )}
                    </div>
                    ...
                    {status.state === 'error' && (
                        <div className="pt-8">
                            ...
                            <button onClick={() => setStatus({ state: 'idle', message: '' })} className="mt-4 bg-sky-500 text-white px-4 py-2 rounded">{t('retryButton')}</button>
                        </div>
                    )}
                </div>
            </div>
